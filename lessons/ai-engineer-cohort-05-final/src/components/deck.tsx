...
export const accentSequence = [colors.red, colors.yellow, colors.blue, colors.purple, colors.green, colors.orange];
...
export function DeckFrame({
	...
	return (
		<Slide bg={bg} style={{
			...
			{darkCanvas && <div aria-hidden style={{ position: 'absolute', top: 10, right: 12, width: 132, height: 48, background: colors.white, border: '2px solid #000', zIndex: 1 }} />}
...
export function Panel({ children, bg = colors.white, style }: { children: ReactNode; bg?: string; style?: CSSProperties }) {
	...
	return <div style={{ background: bg, border: `2px solid ${colors.dark}`, borderRadius: 22, boxShadow: `9px 9px 0 ${isDark ? 'rgba(255,87,87,.58)' : 'rgba(255,222,89,.92)'}`, padding: 26, ...style }}>{children}</div>;
}

export function Label({ children, bg = colors.dark, color = colors.white }: { children: ReactNode; bg?: string; color?: string }) {
	return <span style={{ display: 'inline-flex', alignItems: 'center', background: bg, color, border: `2px solid ${colors.dark}`, borderRadius: 7, padding: '7px 12px', fontFamily: fonts.mono, fontWeight: 800, fontSize: 15, letterSpacing: .35 }}>{children}</span>;
}
...
export function LargeQuote({ children, accent = colors.yellow }: { children: ReactNode; accent?: string }) {
	return (
		<div style={{ position: 'relative', background: colors.dark, color: colors.white, border, boxShadow: `10px 10px 0 ${accent}`, padding: '32px 42px' }}>
			<div aria-hidden style={{ position: 'absolute', top: -28, left: 24, fontFamily: fonts.heading, fontSize: 90, color: accent, lineHeight: 1 }}>“</div>
			<div style={{ fontFamily: fonts.heading, fontWeight: 800, fontSize: 34, lineHeight: 1.35 }}>{children}</div>
		</div>
	);
}
...
export function RoleFocusSlide({
	...
	return (
		<DeckFrame tag={`ROLE ${index}`} title={title} subtitle={problem} accent={accent}>
			<div style={{ display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: 28, height: '100%', alignItems: 'stretch' }}>
				<AnimatedGroup delay={0.18} style={{ display: 'flex' }}>
					<Panel style={{ flex: 1, borderTop: `12px solid ${accent}` }}>
						...
						<div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginTop: 26 }}>
							{capabilities.map((item, i) => <div key={item} style={{ fontSize: 27, fontWeight: 800, padding: '18px 14px', background: i % 2 ? '#fff8f2' : '#fff', borderBottom: `5px solid ${accent}` }}>{item}</div>)}
						</div>
					</Panel>
				</AnimatedGroup>
				<AnimatedGroup delay={0.28} style={{ display: 'flex' }}>
					<Panel bg={colors.dark} style={{ flex: 1, color: colors.white }}>
						<Label bg={colors.yellow} color={colors.black}>面试会继续追问</Label>
						<div style={{ marginTop: 28, display: 'grid', gap: 22 }}>
							{interview.map((item, i) => <div key={item} style={{ display: 'flex', gap: 14, alignItems: 'flex-start', fontSize: 25, lineHeight: 1.35, fontWeight: 700 }}><NumberBadge bg={accent}>{i + 1}</NumberBadge><span>{item}</span></div>)}
						</div>
					</Panel>
				</AnimatedGroup>
			</div>
...
export { colors, fonts, border, shadow };
