...
const choices = [
	{ letter: 'A', label: '一股 AI 味 / 没法直接发', sub: '写出来像机器人，每次还得自己大改', bg: colors.green, rotate: -2, emoji: '😮‍💨' },
	{ letter: 'B', label: '效率翻倍 / 已经离不开', sub: '选题、写稿、配图全靠它', bg: colors.orange, rotate: 2, emoji: '🔥' },
] as const;
		...
		<Slide bg={colors.warmBg}>
			<div style={{ width: '90%', maxWidth: 1400, height: '85%', padding: '40px', display: 'flex', flexDirection: 'column' }}>
				<motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 28 }}>
					<span style={{ padding: '6px 16px', background: colors.dark, color: colors.white, fontFamily: fonts.mono, fontSize: 16.5, fontWeight: 700, border, boxShadow: `3px 3px 0 ${colors.rose}` }}>POLL · 01</span>
					<span style={{ fontFamily: fonts.mono, fontSize: 16.5, fontWeight: 700, color: '#666' }}>现在请认真想 10 秒</span>
				</motion.div>

				<motion.h2 initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15, duration: 0.45 }}
					style={{ fontFamily: fonts.heading, fontSize: '65.5px', fontWeight: 900, lineHeight: 1.15, letterSpacing: -1.5, color: colors.black, marginBottom: 12 }}>
					你用 <span style={{ background: colors.yellow, padding: '0 14px', border, boxShadow: shadow, display: 'inline-block', transform: 'rotate(-1deg)' }}>AI 做营销内容</span>
				</motion.h2>
				<motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.25, duration: 0.45 }}
					style={{ fontFamily: fonts.heading, fontSize: '65.5px', fontWeight: 900, lineHeight: 1.15, letterSpacing: -1.5, marginBottom: 36 }}>
					用得怎么样？
				</motion.div>
				...
				<div style={{ flex: 1, display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 32, alignItems: 'center' }}>
					{choices.map((c, i) => (
						<motion.div key={c.letter} initial={{ opacity: 0, y: 50, rotate: 0 }} animate={{ opacity: 1, y: 0, rotate: c.rotate }}
							...
							<div style={{ fontFamily: fonts.heading, fontSize: '34px', fontWeight: 900, lineHeight: 1.2, marginBottom: 10, color: colors.black }}>{c.label}</div>
