...
| **PERSONAS.md** | `curriculum/ai-programming/PERSONAS.md` | `target-user-persona-mapper` + Mason + Summer | 销售页 hero / xhs 切角 / 顾问话术 / funnel 决策周期 | 🟡 v0.5 enhanced GT 0.55（阈值刚过，待升 0.75）|
| **PERSONAS-INIT-REPORT.md** | 同目录 | persona-mapper init dogfood 产出 | Lightman 看对账 | ✅ 已写 |
| **FUNNEL_PLAN.md** | `curriculum/ai-programming/FUNNEL_PLAN.md` | `course-funnel-architect` + Aurora + Mason | promotion / 销售页定价 / 引流课 | ✅ v0 init 推荐选项 B（¥99 引流 + 现 2 档），等拍板 |
...
| **AUDIT_LOG.md** | `curriculum/ai-programming/AUDIT_LOG.md` | `bootcamp-sync` 操作者 / Mason | 翻车追责 / cohort 复盘喂回 PERSONAS refresh | 🟡 有 2026-05-12 _id 重对齐 1 条，无 cohort 02 复盘 |
...
| 2 | **学员侧 ground truth 采集 0 机制**（PERSONAS / 作品 / Testimonial / NPS）| A/C/D/E | 🔴 P0 | Mason | 02 期已开课 = 最佳窗口，错过断崖 |
| 3 | **Cohort 学员侧 onboarding / 运营 / 留存 SOP 全套 0**（B→C→D 三段裸奔）| B/C/D | 🔴 P0 | Beta + Mason | 03 期开班前 1 周必须 ship |
...
| 2 | P0 | 2-3d | 新建 `cohort-graduate-portfolio-collector` skill（02 期作品采集）| Mason | 🔴 未启动 | L-28 前必须 ship（02 期结业窗口正在关）|
| 3 | P0 | 1w | 新建 cohort 三件套 skill（launch-sop / daily-ops / progress-tracker）| Mason + Beta | 🔴 未启动 | L-7 前 ship |
| 4 | P0 | 1w | 数据系统：cohort + attribution + referral 三 collection + admin Content ROI tab | Dev | 🔴 未启动 | L+0 前 ship 1/3，L+14 全部 ship |
| 5 | P0 | 2-3d | Mason 强制完成 5 个 02 期学员访谈 + Summer 扫 xhs 评论 → PERSONAS GT 0.55→0.75+ | Mason + Summer | 🟡 skill 齐全没排时间 | 本周内启动 |
...
| Mon | 🚨 **Mason + 顾问访谈 × 3 拉 PERSONAS GT 0.55→0.75** | Mason | `INTERVIEW_SCRIPT.md` |
| Tue | Summer 跑 `/persona-ground-truth-scraper init --source=xhs,wechat` | Summer | `persona-ground-truth-scraper` |
| Tue | Mason 启动 `cohort-graduate-portfolio-collector` 给 02 期发授权同意表 | Mason | 新 skill #2（先跑手动版）|
...
| Day | Task | Owner | Skill / SOP |
|-----|------|-------|------------|
| Mon | 销售页 Mode B 升级：加 OFFER WALL + Faculty 占位 + 03 期 commenceCourseDate banner | Dev + Mason | `course-custom-landing` Mode B |
| Mon | 海报第 1 套（主色 #FF7B5A）+ user-test | Designer + Marketing | `poster-user-test` |
| Tue | 第 1 场讲座 topic 审「零基础 30min 做出第一个网页」 | Mason | `webinar-topic-feasibility` |
...
| Tue 19:30 | 第 1 场讲座（试听性质，现场做出一个网页）| Mason | `classroom-deck-builder`（kickoff-ceremony mode）|
...
| Mon | `cohort-launch-sop`：拉群 SOP + Welcome Email + 班委选举模板上线 | Beta + Mason | 新 skill #3a |
| Tue | T-7 公开试听讲座（销售导向）| Mason + Aurora | `classroom-deck-builder` |
| Tue | EOI 第 2 波 24h SLA | Neomi + 顾问 | `eoi-followup` |
| Wed | `cohort-readiness-check`：T-7 实测 18 Lab + 33 工具是否还能用 | Mason | `curriculum-review`（cohort-readiness mode 扩展）|
...
|------|------|-------|------------|
| 09:00 | 开学典礼 deck 准备 | Mason + Beta | `classroom-deck-builder`（kickoff-ceremony mode 新扩展）|
| 19:30 | 开学典礼直播（破冰 + 仪式感 + Faculty 亮相 + Q&A）| Mason | 同 |
...
| D+30 | Aurora 跑 `course-promotion-architect postmortem`（学员侧 + 课程侧 + 漏斗侧三层）| Aurora + Mason | 同 |
| D+30 | FUNNEL_PLAN.md 真实转化率回写（行业基线 5% → 03 期真值）| Mason | `course-funnel-architect` postmortem |
| D+30 | PERSONAS.md refresh（用 03 期真实学员数据）| Mason | `target-user-persona-mapper refresh` |
| D+30 | 04 期 init（喂 03 期数据进 GAP_AUDIT v2 + 本 MASTER v2）| Aurora + Mason | 本文件 update |
...
[Mason + Summer 拉 PERSONAS GT 0.55→0.75] (P0 · 2-3d)
       │
...
## §4. 决策点（必须 Lightman / Mason 拍板的事）

> 这 8 个决策每多卡 1 天 = 整条链路停 1 天。

| # | 决策 | 谁拍 | 卡了什么 | 卡了多久 | 备注 |
|---|------|------|---------|---------|------|
| 1 | **03 期 launch date** | Lightman + Mason | 整个时间轴 / 销售页 banner / 7d nurture | 自 2026-05-28 起 | 拖 1 周 = 招生窗口 -1 周 |
...
| 4 | **是否做 ¥99 引流课**（FUNNEL_PLAN 选项 B）| Lightman + Mason | Phase 0 是否拆出独立卖 / 7d 过渡能否启动 / 流量入口宽度 | 自 2026-05-29 起 | ROI 推测 4-5x，但需 Phase 0 拆课 1 周 |
...
| 7 | **Faculty 实名是谁**（销售页加 Faculty section）| Mason + Lightman | 销售页信任建设（ai-engineer 有 5 张校友 OFFER，本课全空）| 自 2026-05-29 起 | 至少 2-3 人 |
| 8 | **是否启动 PBL 项目融入 ai-programming**（与 ai-engineer 拉开档次）| Lightman + Mason | 进阶班是否升级到包含 PBL | 长期决策 | 影响定价 + 招生定位 |
...
| 课程主理人 | **Mason** | PERSONAS 验证 / 讲座主持 / 销售页定位 / 学员访谈 / cohort 设计 | ✅（bandwidth 紧）|
...
| 缺岗 | 严重度 | 替代方案 | ETA |
|------|--------|----------|------|
| Marketing 文案 | 🔴 P1（不补 → 4 条产出链全延期）| 短期：Aurora / Mason / Seren 兼；中期：兼职 freelancer | 1 周内 |
...
| OFFER WALL 张数 | 0 张 | 5+ 张 / cohort | 02 期学员授权 + content.ts | 一次性 | Mason |
| PERSONAS GT 覆盖率 | 0.55 | 0.75+ | 数 GT sources 数量 / weight | 月 | Mason |
...
| W3 / 结业 / 30 天 NPS | ⚠️ 0 数据 | NPS ≥ 50 | `cohort-nps-collector` 问卷 | event | Beta + Mason |

### 6.4 Phase D · 课后复盘续费 KPI

| KPI | baseline | target | 数据源 | 频次 | 谁取 |
|-----|----------|--------|--------|------|------|
| 4 大交付物完成率 | ⚠️ 未追 | 60%+ | `cohort-graduate-portfolio-collector` | event | Mason |
...
> 启动前提：本 MASTER 文档已被 Lightman / Aurora / Mason 三人 review 过。
...
| 10:00 - 11:00 | **Mason 启动 `cohort-graduate-portfolio-collector` 给 02 期学员发授权同意表 + 4 大交付物收集表单** | Mason | 02 期 24 学员收到表单 |
| 11:00 - 12:00 | Mason 约 Amelia / Rain / Angela 3 个 INTERVIEW 时段（本周内排满）| Mason | 3 个 30min 会议时段 |
...
| 14:00 - 15:00 | Mason INTERVIEW 第 1 场（Amelia）| Mason + Amelia | `gt-sources/interview-sales-amelia.md` |
...
| Time | Task | Owner | Output |
|------|------|-------|--------|
| 09:00 - 10:00 | Aurora + Mason 决策 #4：是否做 ¥99 引流课 | Aurora + Mason + Lightman | 拍板（本文件 §4 update）|
...
| 14:00 - 15:00 | Mason INTERVIEW 第 2 场（Rain）| Mason + Rain | `gt-sources/interview-sales-rain.md` |
...
| Time | Task | Owner | Output |
|------|------|-------|--------|
| 09:00 - 12:00 | Dev + Mason 跑 `course-custom-landing` Mode B：加 OFFER WALL 占位 + Faculty 占位 + 03 期 commenceCourseDate banner | Dev + Mason | 销售页 content.ts update PR |
| 10:00 - 16:00 | 文案（或兼任）写公众号长文 1「不会代码做出可上线产品」用 `blog-longform-writer` + `wechat-article-quality` 审 | 文案 | 长文 + 评分 ≥ 80 |
| 14:00 - 15:00 | Mason INTERVIEW 第 3 场（Angela）| Mason + Angela | `gt-sources/interview-sales-angela.md` |
| 16:00 - 17:00 | Summer 把 2 天前 scraper 结果 + 3 场访谈语料合并 → 跑 `/target-user-persona-mapper validate ai-programming` | Summer + Mason | PERSONAS.md GT 0.55 → 0.75 |
...
| 11:00 - 12:00 | 02 期学员产出回收进度对账（Mason vs 表单实际收到数）| Mason | 收到张数 / 24 |
...
| PERSONAS GT 升级（0.55→0.75）| §1.2 gap #2 状态 update / §1.3 影响重审 | Mason |
| 数据系统 collection ship | §6.6 状态 update / §1.4 Top 10 中相关项 ETA update | Dev |
| 每周一晨会 | §1.4 Top 10 owner / status / ETA update | Aurora |
| 每月末 | 跑 `course-promotion-architect audit ai-programming` 复盘 5 phase 覆盖率 → §1.1 update | Aurora + Mason |
| 03 期开课 30 天后 | 用真实数据 refresh PERSONAS + FUNNEL_PLAN + 本 MASTER → 喂 04 期 init | Mason + Aurora |
...
**Q6: 02 期学员我能采访谁？**
A: Mason 手里有 cohort 名单。优先采访：(a) 完成 4 大交付物的 (b) 在群里活跃的 (c) 自我介绍说「零基础」的。每个 30 min INTERVIEW。
