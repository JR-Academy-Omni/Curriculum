...
| 交付物 | 谁产出 | 落盘路径 | 谁消费 |
|--------|--------|---------|--------|
| 02 期 STATE.md（追溯建档）| Mason | `curriculum/ai-programming/cohort-02/STATE.md` | Aurora 复盘 / 03 期参照 |
| 03 期 STATE.md（开班前建好）| Mason + Aurora | `curriculum/ai-programming/cohort-03/STATE.md` | 日常运营单一真相文档 |
| 02 期学员产出包（截图 + 1 句话 + 同意）| Mason 收 / Beta 助教协助 | `curriculum/ai-programming/student-outputs/cohort-02/` | OFFER WALL / xhs / 公众号 |
...
2026-06-09 Tue-06-12 Fri ◆ 内容 + 03 期招生预热
        │  ├─ 公众号长文 2「02 期校友零基础做出 X」（文案岗或临时 Mason 顶）
...
**落地动作**：Lightman 直接编辑 `MARKETING_WORKFLOW_MASTER.md` §0.1 表格 + Slack 通知 Aurora / Mason / Neomi。

### 2.2 Mason 10:00-17:00：启动 02 期学员产出收集
...
**会议参与人**：Aurora / Seren / Summer / Lily / KIKI / Mason / Neomi / Lightman（旁听）

**Agenda**（基于 MASTER + GAP_AUDIT）：

1. **状态同步（10 min）**：5 phase 覆盖率（A 71% / B 62% / C 7% / D 42% / E 39%）
2. **本周派单（30 min）**：W1 13 个 P0/P1 task 分给具体人
3. **02 期复盘启动（15 min）**：Mason 介绍 STUDENT_OUTPUT_COLLECTION_SOP
...
| W1 02 期学员产出收集 5 件 | Mason + Beta 助教 | STUDENT_OUTPUT_COLLECTION_SOP | 周五 16:00 |
| W1 02 期班级群运营 daily | Mason | COHORT_GROUP_OPERATIONS_SOP | 每日 |
| W1 alumni 推 03 期 first batch | Aurora + Mason | ALUMNI_NURTURE_LOOP | 周四 |
...
- 下午 Mason + Aurora 起草"老学员专属优惠 + 推荐人激励"私信
...
Mason 给 02 期发 NPS 调研问卷（不强制 + 不交换福利）：
...
- 14:00 Mason 看 cohort STATE.md → cohort-ops-conductor

### 7.2 每日 cron 节奏

- 08:00 marketing-data-collector 自动跑
- 09:00 Mason 看 02/03 期班级群 + 早报
...
| 断裂 | 描述 | 修复位置 | 修复手段 |
|------|------|---------|---------|
| B → C | EOI lead 数据 / 入群名单没传给运营 | W1 周二周会 | Aurora 把 EOI 字段加进 STATE.md schema，Neomi 派单时同步通知 Mason |
...
|-----------|---------|--------|---------|
| **Phase C 黑洞继续扩大** | 02 期班级群 SOP W1 没跑通（连续 2 天漏运营）| Mason | COHORT_GROUP_OPERATIONS_SOP §应急 |
| **数据回收失效** | daily metrics W1 跑 ≤ 3 天 | Aurora | marketing-data-collector DATA_SOURCES.md 优先补缺源 |
| **学员产出 0 入库** | W1 末入库 < 3 件 | Mason | STUDENT_OUTPUT_COLLECTION_SOP §3 触达话术升级 |
| **Marketing 文案岗 W1 末 0 应聘** | JD 发出 5 天 0 简历 | Lightman + HR | 重写 JD + 加薪资范围 + 朋友圈 + 朋友群 |
| **跨 cohort 学习路径继续断** | alumni-nurture W1 末 < 10 条私信发出 | Aurora + Mason | ALUMNI_NURTURE_LOOP §应急简化版 |
...
- **本文件由谁维护**：Aurora（主） + Mason（02/03 期运营侧补充） + Lightman（决策点签字）
...
**Aurora + Mason 推荐**：选项 B（2026-06-29），理由：
