...
> 维护人：Beta（课程主理人）+ Aurora（Marketing 主管）+ Beta（教务）
...
- **校验**：Beta 点开 URL 看 30 秒，能讲清楚做的什么就算 pass
...
- **典型场景**：
  ...
  - 转型者：副业服务的展示页
  - 学生：作品集主页
- **校验**：Beta 点开 URL + 检查 view-source 看是不是抄的课程 demo，跟 Lighthouse 跑一遍移动端
...
**执行**：Beta 在班级群发一条公告（不是私聊，本节是唯一的群发，后面全部 1v1）
**话术核心**：
- 提到结业后会收作品做"校友墙"
...
**执行**：Beta 私聊 P0 学员，话术参考 §3.1 模板
**预期**：5-8 人响应（02 期 37 人 × P0 估 20% = 7 人）

### 2.4 D+35（第 5 周末）· 第二次 1v1 邀请

**目的**：P1 学员补料 + P0 没响应的二次接触
**执行**：Beta + Beta 分头跑（Beta 接 P0 未响应，Beta 接 P1 引导补料）
**预期**：累计响应到 12-15 人

### 2.5 D+42（第 6 周末 = 答辩前一周）· 强提醒

**目的**：明确告诉学员"答辩后 7 天内是收集窗口期"
**执行**：Beta 班级群发一条总结公告（这是第 2 次也是最后一次群发）+ 同时把 release 同意书发到每个学员邮箱

### 2.6 D+45（结业答辩当晚）· 立即收

**目的**：答辩当晚 emotional high 是同意率最高的窗口，必须借势
**执行**：
- 答辩结束后 30 分钟内，Beta 在群里贴 Google Form 链接
...
**流程**：
1. Beta 1v1 教学员 fork curriculum repo
...
|------|-------------------|---------|---------|---------|
| 1v1 私聊 → Form | 15-20 人 | Beta 5 min/人 | 中高 | D+28 / D+35 / D+45 |
| 答辩当晚 Form 批量 | 5-8 人 | 学员 10 min | 中 | D+45 当晚 |
| git PR | 2-3 人 | Beta 30 min/人 | 极高（含真实 commit history）| 全程开放 |
...
- [ ] 答辩录像剪辑成 cohort vibe 视频（Beta 主导）
...
- Beta 是该目录默认 reviewer（除答辩当晚批量 import 由 Beta 直接 merge 加速）
- 学员个人 PR 进来时 Beta / Beta 必须 review consent.json 字段是否跟学员实际勾选一致
...
|------|------|------------|-----------|
| **课程主理人** | Beta | 1v1 私聊学员（D+28/D+35/D+45）/ release 内容审定 / 答辩当晚收集 / 决定哪些上 OFFER WALL highlight | git PR review |
...
| **新媒体** | Summer | 小红书帖子配图筛选（基于学员勾选 channels.xhsPost=true）/ 撤回 24h SLA 执行 | 帮 Beta 翻 ground truth |
...
D+14 Beta 群发预告 → D+21 Beta 内部盘点 → D+28 Beta 1v1 P0 → D+35 Beta+Beta 1v1 P1 → D+42 Beta 群提醒 + 发同意书 → **D+45 答辩当晚集体行动**：Beta 主持 / Beta 整理 Form 数据 / Designer 现场拍照 → D+50 Aurora sign-off 哪些进销售页 → D+55 Dev push ALUMNI_WORKS 到 content.ts → D+60 Beta 二次唤醒 + Beta 更新 lastVerifiedAt → 月度 Beta + Aurora review 撤回情况。

### 7.2 失败兜底

- Beta 太忙拖到 D+50 还没动 → Beta 触发，Aurora 知会 Lightman
...
这份 postmortem 直接 feed 给 03 期开课时 Beta 用，避免每期冷启动。
...
- ❌ 学员撤回后超过 SLA 还没下线 → Beta 视为 P0 紧急事件，立即叫 Dev 处理
...
conductor 检查 cohort-02/ 进度 → 判断当前日期 D+N → 触发对应阶段
  ↓
- D+14: 提示 Beta 群发预告（template 在本 SOP §2.1）
- D+21: 触发 Beta 跑 _progress-21d.md 自动盘点（接 jr-academy MongoDB 拉 Lab/Quest 提交数据）
- D+28: 提示 Beta 1v1 P0 学员名单（话术 template §3.1）
- D+35: 提示 Beta+Beta 1v1 P1 学员
- D+42: 提示 Beta 群提醒 + Beta 邮件 release 同意书 PDF
- D+45: 启动 Form 批量收集流程
...
- 触发时机表（§2）→ 决定今天该提示谁做什么
- 团队分工（§7）→ 决定提示发给谁
- KPI 目标（§8）→ 决定是否亮红灯（如 D+50 还没收到 5 人 → 紧急叫 Beta）
...
| — | 02 期回收启动日期待定 | 等 Beta 确认 02 期实际答辩日 + 倒推 D+28 |
