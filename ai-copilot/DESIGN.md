# AI 数据分析 & Copilot 自动化实战 — Design Spec

> Excel + Copilot·数据分析·自动化
> 视觉系统所属：**垂直行业线（数据/办公）**
...
```
设计任务：为「AI 数据分析 & Copilot 自动化实战」做配图 / 海报 / banner / 封面图。
课程定位：Excel + Copilot·数据分析·自动化（垂直行业线（数据/办公））
...
Excel/数据/办公自动化 = 金黄琥珀色。和 Essentials 的 JR 黄区分（更暖更深），不会撞色。
...
> 在 HTML/Slide 里使用时，以变量形式声明：`--theme-primary: #F59E0B;`
