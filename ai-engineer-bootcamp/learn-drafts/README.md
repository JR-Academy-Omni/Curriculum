...
1. **放内容**:把 `ai-engineer/*.md`(共 **4 篇**)复制进 `jr-academy-web-zh/src/content/learn/ai-engineer/`,并按 `src/config/learn/directions/ai-engineer.config.ts` 现有条目的字段格式补 **4 条** config(对齐 frontmatter,order/group 按实际排)。上线后确认 `/learn/ai-engineer/{slug}` **四个**页面可访问。
2. **连大纲**:页面上线后,在 `curriculum/ai-engineer-bootcamp/public/outline.json` 对应 Phase 插入 **4 节** `自学` lesson(片段见下),再跑 `python3 lineage/build_lineage.py` 重建谱系,`/bootcamp-sync` 同步到 production。
...
},
{
  "code": "L163y",
  "type": "Information",
  "title": "自学：开源模型、本地部署与模型路由",
  "isLive": false,
  "duration": 25,
  "description": "开源模型怎么选、Ollama vs vLLM、自托管 vs API 的成本账、简单请求走小模型省钱的路由。",
  "learns": [{ "direction": "ai-engineer", "slug": "open-weight-models-local-deploy" }]
}
```

> 建议插在 Open-Weight Models + Model Routing 直播课 / 那个 Ollama Quest 之后,和微调篇相邻。
