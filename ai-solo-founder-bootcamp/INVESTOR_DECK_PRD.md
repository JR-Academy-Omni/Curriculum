...
- **必含要素**：
  - co-brand logo lockup：匠人 `https://jiangren.com.au/icon/logo-zh-white.svg`（深色封面）× `[投资人 / 机构 LOGO]`（占位，他们的）。
...
- **Logo**：封面深色 `https://jiangren.com.au/icon/logo-zh-white.svg`（真实 served logo，**不用 `[LOGO]` 占位**）；投资人 / 机构方 logo 用 `[投资人 LOGO]` 占位（那是他们的）。不拉伸 / 不加 shadow / 不红底放红 logo。
